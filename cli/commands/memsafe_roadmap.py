...
def memsafe_roadmap_command(args: argparse.Namespace) -> int:
    ...
    # --- Emit run.start for streaming consumers (web UI, log ingestion) ------
    ...
    events.stage("render_roadmap", index=2, total=total_stages)
    ...
    if args.recompose and args.approve:
        ...
        if not approval_prompt(
            f"ADR ready at {root_adr_path}. Continue to recompose to Rust?",
            yolo=args.yolo,
        ):
    ...
    if args.recompose:
        ...
        events.stage("recompose", index=recompose_idx, total=total_stages, target="Rust")
        ...
        if rc != 0:
            events.warn(f"recompose exited {rc}; roadmap + ADRs already written")
            output.warn(style, f"recompose exited {rc}; roadmap + ADRs already written")
        else:
            events.stage_done("recompose", rust_dir=str(rust_dir))

    # Final structured result (always emitted; streaming consumers rely on it)
    events.result(
        0,
        roadmap=str(roadmap_path),
        adr_root=str(root_adr_path),
        ...
        ownership_decisions=len(ownership),
        used_domain_schema=use_domain,
        rust_output=str(rust_dir) if rust_dir else None,
        review_findings=review_result if review_result else None,
    )

    # Human-mode summary — only render when NOT in ndjson/streaming mode.
    if events.get_mode() != "ndjson":
        if args.format == "json":
            print(json.dumps({
                "ok": True,
